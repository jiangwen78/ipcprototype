...
import OkToSettle from "./components/OkToSettle.vue"
import RestaurantNumber from "./components/RestaurantNumber.vue"
import DocumentStatus from "./components/DocumentStatus.vue"
import FranchiseAgreementId from "./components/FranchiseAgreementId.vue"
...
// {
//     "searchText": "",
//     "restaurantStatus": [], // will be the long integer value for the chosen status(es)
//     "franchiseAgreementStatus": [], // will be the long integer value for the chosen status(es)
//     "documentStatus": [], // will be the long integer value for the chosen status(es)
//     "pagenumber": 0 // page number
//     "pagesize": 10
// }

const loadData = async (payload) => {
  const url = "/mock-api-data/restaurants.json"
...
const columnDefConfig = [
  {
    field: "okToSettle",
    cellRenderer: "OkToSettle",
  },
  {
    field: "restaurantNumber",
    cellRenderer: "RestaurantNumber",
  },
  {
    field: "address",
    cellClass: "restaurant-address",
    cellRenderer({ value }) {
      if (!value) {
        return ""
      }

      return `${value.line1} ${value.line2 || ""} ${value.city} ${
        value.state
      } ${value.postcode} ${value.country}`
    },
  },
  {
    field: "documentOnFile",
    cellRenderer: "DocumentStatus",
  },
  {
    field: "franchiseAgreementId",
    minWidth: 320,
    cellRenderer: "FranchiseAgreementId",
  },
  {
    field: "dividendPayment",
...
const columnFields = {
  ...
  sm: ["restaurantNumber", "franchiseAgreementId", "action"],
}
...
const franchiseAgreementStatusOptions = [
  {
    label: "active",
    value: "active",
  },
...
export default {
  ...
  data() {
    return {
      ...
      franchiseAgreementStatusOptions,
      ...
      filterConfig: [
        ...
        {
          key: "franchiseAgreementStatus",
          title: "Franchise Agreement Status",
          options: franchiseAgreementStatusOptions,
        },
        {
          key: "documentStatus",
      ...
      <div class="ag-header">
        ...
        <div class="searchbar">
          ...
          <p class="searchbar-tips">
            Search by restaurant number, city, or state
          </p>
        </div>
      </div>
    </div>
          ...
          <div class="filterbar-content">
            <div class="filterbar-item">
              <filter-select
                ...
                :options="restaurantStatusOptions"
                v-model="filter.restaurantStatus"
                @change="submitFilter"
              />
            </div>
            <div class="filterbar-item">
              <filter-select
                placeholder="Franchise Agreement Status"
                :options="franchiseAgreementStatusOptions"
                v-model="filter.franchiseAgreementStatus"
                @change="submitFilter"
              />
            </div>
            <div class="filterbar-item">
                ...
                <div class="filter-popup-item-body">
                  <div class="filter-popup-select-option-list">
                    <div
                      ...
                      v-for="option in restaurantStatusOptions"
                      :key="option.label"
