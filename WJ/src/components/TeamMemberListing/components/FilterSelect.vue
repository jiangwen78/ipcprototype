...
<template>
  <div
    :class="`select-box select-box==${props.size} ${opened ? 'opened' : ''}`"
    @click="handleSelectBoxClick"
  >
    <div class="select-content" @click="toggleOpenStatus">
      <div class="select-placeholder">{{ props.placeholder }}</div>
      <div class="select-count" v-show="props.modelValue.length">
